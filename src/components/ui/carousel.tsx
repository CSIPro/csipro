...
interface CarouselNavigationProps extends React.HTMLAttributes<HTMLDivElement> {
  name: string;
}
  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        <CarouselDot
          key={`${name} carousel slide ${index + 1}`}
...
interface CarouselDotProps extends React.HTMLAttributes<HTMLButtonElement> {
  isActive?: boolean;
}
